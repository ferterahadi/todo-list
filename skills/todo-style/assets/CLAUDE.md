...
Always show before and after, never prose about a change. The whole CHOICES block still comes last — the accordion sits above the `## ➡️ YOUR CALL` banner, never between the banner and the picker.
...
- If the harness offers an interactive picker tool, fire it with the same options and the same recommendation after the `### ➡️ Choose` table, so I can click instead of type.
